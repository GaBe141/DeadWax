...
- Runtime controls are defined in `scripts/main.gd`: A/D or arrows move, Space jumps, J/X strikes, hold K/C raises the Hood, hold L kneels/Sets, E/gamepad Y enters passages, I/gamepad Start opens The Book, and R respawns at the active entry. Tab cycles rooms and M toggles the grayed-in planned world, both only in debug builds.
