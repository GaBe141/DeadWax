...
static func draw(canvas: Node2D, pose: Dictionary, ink: Color, stock: Color, font: Font, type_size: int) -> void:
	...
	var warm := Color("b07b32") if stock.get_luminance() > 0.45 else Color("efc774")
	...
	if guarded:
		for side in [-1.0, 1.0]:
			...
			canvas.draw_polyline(bracket, Color(stock, 0.9), 5.5 + blocked, true)
			canvas.draw_polyline(bracket, Color(ink, 0.75 + blocked * 0.25), 2.5 + blocked * 1.5, true)
	elif open:
