...
func set_snapshot(value: Dictionary) -> void:
	var strike_unlocked := bool(value.get("strike_unlocked", true))
	var chain_unlocked := strike_unlocked and bool(value.get("chain_unlocked", true))
	var step := clampi(int(value.get("step", 0)), 0, 3 if chain_unlocked else 1)
	var cooldown := maxf(float(value.get("cooldown_remaining", 0.0)), 0.0)
	var window := maxf(float(value.get("window", 0.65)), 0.001)
	var remaining := clampf(float(value.get("remaining", 0.0)), 0.0, window)
	var active := strike_unlocked and step > 0 and (remaining > 0.0 or (not chain_unlocked and cooldown > 0.0))
	...
	if not strike_unlocked:
		input_state = "locked"
	elif input_state not in ["ready", "recover", "buffer", "queued", "blocked"]:
		input_state = "blocked"
	_snapshot = {"step": step if active else 0, "remaining": remaining if active else 0.0,
		...
		"cooldown_remaining": cooldown, "strike_unlocked": strike_unlocked, "chain_unlocked": chain_unlocked,
		"cooldown_duration": maxf(float(value.get("cooldown_duration", 0.2)), 0.001),
		"queued": strike_unlocked and bool(value.get("queued", false))}
	if fresh or not active or input_state in ["queued", "blocked", "locked"]:
		_early_t = 0.0
...
func _apply_snapshot() -> void:
	...
	for index in _beats.size():
		...
		label.visible = strike_unlocked and (chain_unlocked or index == 0)
	...
	_window.visible = chain_unlocked and step > 0
	_apply_stamp()

func _input_text() -> String:
	if not bool(_snapshot.strike_unlocked):
		return "Explore the Headshell"
	match String(_snapshot.input_state):
		"queued":
			if not bool(_snapshot.chain_unlocked):
				return "QUEUED · TAP"
