extends RefCounted
## Authored campaign only. The chapter factories retain their own rooms;
...
const UnplayedChapter := preload("res://scripts/chapter_three.gd")
...
static func room_ids() -> Array[StringName]:
	...
	ids.append_array(UnplayedChapter.room_ids())
	return ids

static func has_room(id: StringName) -> bool:
	return LabelChapter.has_room(id) or OvertureChapter.has_room(id) or UnplayedChapter.has_room(id)

static func create_room(id: StringName) -> Node2D:
	if LabelChapter.has_room(id):
		return LabelChapter.create_room(id)
	if OvertureChapter.has_room(id):
		return OvertureChapter.create_room(id)
	return UnplayedChapter.create_room(id)
...
static func chapter_label(id: StringName) -> String:
	if UnplayedChapter.has_room(id):
		return "THE SCRATCH" if id == &"the_drop" else "THE UNPLAYED"
