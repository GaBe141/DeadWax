...
static func _drop(c: CanvasItem, size: Vector2, far: bool, clock: float, p: Dictionary) -> void:
	...
	Brush.motes(c, Rect2(size.x * 0.25, 170, size.x * 0.5, size.y - 260), clock, p.copper, 9)

static func _landing(c: CanvasItem, _size: Vector2, far: bool, clock: float, p: Dictionary) -> void:
	var center := Vector2(980, 355)
	if far:
		Brush.archway(c, center + Vector2(0, 15), Vector2(492, 348), 659, _faint(p, 0.16), 24)
		return
	...
	Brush.motes(c, Rect2(center.x - 354, 166, 708, 347), clock, p.gold, 6)
...
static func _verse(c: CanvasItem, size: Vector2, far: bool, clock: float, p: Dictionary) -> void:
	...
	Brush.motes(c, Rect2(184, 172, size.x - 360, 354), clock, p.light, 6)

static func _warren(c: CanvasItem, size: Vector2, far: bool, clock: float, p: Dictionary, lower: bool) -> void:
	if far:
		# The distant painting supplies the room mass; keep this plane open.
		return
	for i in range(0, 7, 2):
	...
	Brush.motes(c, Rect2(166, 175, size.x - 330, size.y - 360), clock, p.copper, 7)
...
static func _gallery(c: CanvasItem, _size: Vector2, far: bool, clock: float, p: Dictionary) -> void:
	...
	Brush.motes(c, Rect2(center.x - 398, 350, 796, 427), clock, p.gold, 5)
