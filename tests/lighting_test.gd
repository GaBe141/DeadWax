...
func _check_room_lights() -> void:
	...
	for id in CampaignScript.room_ids():
		...
		_check(lighting.ambient.color == lighting.profile.ambient, String(id) + " A-side ambient equals its authored profile even with light ink on dark stock")
		...
		_check(lighting.ambient.color == lighting.profile.ambient.lerp(Color.WHITE, 0.25), String(id) + " only the actual B-side swap receives the quarter-strength fill")
